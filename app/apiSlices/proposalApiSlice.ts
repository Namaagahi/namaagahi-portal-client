...
const proposalApiSliceTag: any = apiSlice.enhanceEndpoints({addTagTypes: ['Proposal']})
...
export const proposalsApiSlice = proposalApiSliceTag.injectEndpoints({
  ...
  endpoints: (builder: any) => ({
    ...
    deleteProposal: builder.mutation({
      ...
      invalidatesTags: (result:any, error:any, arg:any) => [{ type: 'Proposal', id: arg.id }]
    })
  }),
})

export const { useGetAllProposalsQuery, useCreateNewProposalMutation, useDeleteProposalMutation } = proposalsApiSlice

export const selectProposalsResult = proposalsApiSlice.endpoints.getAllProposals.select(undefined)
...
} = proposalAdapter.getSelectors((state: any) => selectProposalsData(state) ?? finalState)
