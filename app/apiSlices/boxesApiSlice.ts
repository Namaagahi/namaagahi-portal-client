  ...
  endpoints: (builder) => ({
    ...
    updateBox: builder.mutation({
      ...
      invalidatesTags: (result, error, arg) => [
        { type: "Box", id: arg.id, boxId: arg.boxId },
      ],
    }),
    ...
    getBoxById: builder.query({
      ...
      providesTags: (result, error, id, boxVersion) => [
        { type: "Box", id, boxVersion },
      ],
    }),

    createStructureAndUpdateBox: builder.mutation({
      query: (initialBox) => ({
        url: "boxes/others",
        method: "POST",
        body: { ...initialBox },
...
export const selectBoxesResult =
  boxesApiSlice.endpoints.getAllBoxes.select(undefined);
...
export const {
  ...
  (state: any) => selectBoxesData(state) ?? initialState
);
