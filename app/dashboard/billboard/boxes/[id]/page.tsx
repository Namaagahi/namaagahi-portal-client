...
const SingleBox = () => {
  ...
  const box: BoxObject = useSelector(
    (state) => selectBoxById(state as BoxObject, id as string) as BoxObject
  );
  ...
  const calcVariableCosts = () => {
    ...
    box?.structures.forEach((item: BoxStructure) => {
      const thisName =
        allStructures.find(
          (rawStructure) => rawStructure.id === item.structureId
        )?.name ?? "";
    ...
    clone.forEach((stucture: any) => {
      stucture.costs.variableCosts.forEach((varCost: any) => {
        variableCostNames2.forEach((varName) => {
          ...
          if (!stucture["myCustomCost"][varName])
            stucture["myCustomCost"][varName] = 0;
          if (varCost.name === varName)
            return (stucture["myCustomCost"][varName] =
              varCost.figures.monthlyCost);
        });
      });
    });
            ...
            <div className="max-h-[30%] bg-secondary dark:bg-darkModeBg overflow-y-auto p-2 w-full">
              <div className="flex flex-col items-end gap-2">
                <div className="flex items-center gap-3">
                  ...
                  <p>
                    {formatNumber(
                      (totalMonthlyCostSum / 30) * newBox?.duration?.diff,
                      ","
                    )}{" "}
                    ریال
                  </p>
