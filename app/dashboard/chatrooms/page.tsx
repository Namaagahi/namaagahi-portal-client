...
} from "@/app/apiSlices/chatroomsApiSlice";
...
const Chatrooms = () => {
  ...
  const { isLoading, isError } = useGetAllChatroomsQuery(undefined, {
    refetchOnFocus: false,
    refetchOnMountOrArgChange: false,
  });

  const allChatrooms: ChatroomObject[] = useSelector(
    (state) => selectAllChatrooms(state) as ChatroomObject[]
  );
  ...
  const chatroom: ChatroomObject = useSelector(
    (state) => selectChatroomById(state, chatroomId) as ChatroomObject
  );
    ...
    return (
      <>
        <div className="flex flex-col justify-center items-center min-h-screen gap-3">
          <p className="text-xl">هیچ چت رومی تعریف نشده است</p>

          <p>
            برای ایجاد چت روم جدید جدید
            <span
              className="text-cyan-300 cursor-pointer"
              onClick={handleNewChatroomModal}
            >
              کلیک کنید
            </span>
          </p>
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        {(isAdmin || isMaster) && (
          <Button onClickHandler={handleNewChatroomModal} title="چت روم جدید" />
        )}
      </div>
