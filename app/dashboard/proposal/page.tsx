...
const Proposal = () => {
  ...
  const { isLoading, isError } = useGetAllProposalsQuery(undefined, {
  ...
  const allProposals: ProposalObject[] = useSelector(
    (state) => selectAllProposals(state) as ProposalObject[]
  );
  ...
  const proposal: ProposalObject = useSelector(
    (state) => selectProposalById(state, proposalId) as ProposalObject
  );
  ...
  if (isError)
    return (
      <div className="flex flex-col justify-center items-center min-h-screen gap-3">
        <p className="text-xl">هیچ پروپوزالی وجود ندارد</p>
