...
const SinglePlanHeading = (props: Props) => {
  ...
  return (
    <>
      <div className="p-2 h-[15%] backdrop-blur bg-black/50 bg-black dark:bg-primary/80 flex items-center justify-between text-white font-bold">
        <div className="flex flex-col gap-2">
          <p>نوع پلن: {plan.mark.name === "regular" ? "عادی" : "پکیج"}</p>
          <p>نام پروژه: {customer?.name}</p>
          <p>برند: {plan?.brand}</p>
          <p>کد پروژه : {plan?.generalProjectCode}</p>
          {plan.status === "done" && plan.projectCodeId ? (
            <p>شناسه کار : {projectCode?.code}</p>
          ) : (
            ""
          )}
        </div>
        ...
        <div className="flex flex-col gap-2 text-sm">
          {plan?.status === "suggested" ? (
            ...
            <Status status={"معلق"} bgColor={"#b56a35"} textColor={"#FFFFFF"} />
          ) : (
