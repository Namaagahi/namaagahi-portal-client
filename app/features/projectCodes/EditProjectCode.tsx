...
const EditProjectCode = (props: Props) => {
  const { handleModal, projectCode } = props;

  const { id } = useAuth();

  const [updateProjectCode, { isLoading, isSuccess, isError, error }] =
  ...
  const onSubmit = async (data: any) => {
    ...
    if (isError) {
      "status" in error! &&
        error.status === 409 &&
        setErrMsg("این شناسه کار قبلا ثبت شده است");
      "status" in error! &&
        error.status === 400 &&
  ...
  if (isSuccess) {
    toast.success(`شناسه کار با موفقیت ویرایش شد.`);
    handleModal();
  }
    ...
    <div className="py-5 px-8 w-full text-black dark:text-white">
      <form className="flex flex-col" onSubmit={handleSubmit(onSubmit)}>
        <div className="flex justify-between items-center">
          <p className="md:text-2xl text-xl font-bold">ویرایش شناسه کار</p>
